...
<body class="bg-[#0f172a] text-white flex flex-col min-h-screen">
    ...
    <nav class="bg-gray-900 border-b border-gray-800 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
        ...
        <div class="flex items-center gap-2 bg-[#0f172a] p-1.5 rounded-xl border border-gray-800">
            <a href="{{ route('admin.analizador.index') }}" class="text-xs uppercase tracking-wider font-bold px-4 py-2 text-gray-400 hover:text-white rounded-lg transition-all">
                Analizador IA
            </a>
            <a href="{{ route('admin.subir') }}" class="text-xs uppercase tracking-wider font-bold px-4 py-2 bg-orange-500 text-white rounded-lg transition-all shadow-md shadow-orange-500/10">
                Gestión Portafolio
            </a>
        </div>
        ...
        <div class="flex items-center gap-4">
            <div class="text-right hidden sm:block">
                <p class="text-xs font-bold text-white">{{ Auth::user()->name ?? 'Administrador' }}</p>
                        ...
                        <tbody class="divide-y divide-gray-800/50">
                            @foreach($trabajos as $trabajo)
                                <tr class="hover:bg-gray-800/20 transition-colors">
                                    <td class="py-4">
                                        <img src="{{ asset('storage/' . $trabajo->caratula) }}" class="w-12 h-12 object-cover rounded-lg border border-gray-800">
                                    </td>
                                    ...
                                    <td class="py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('portafolio.edit', $trabajo->id) }}" class="bg-gray-800 hover:bg-gray-700 text-gray-300 font-bold text-xs px-3 py-1.5 rounded-lg transition-colors">
                                                Editar
                                            </a>

                                            <form action="{{ route('portafolio.destroy', $trabajo->id) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar este proyecto de manera permanente?');" class="inline">
                                                @csrf
                                                @method('DELETE')

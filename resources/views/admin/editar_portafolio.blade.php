...
<body class="bg-[#0f172a] text-white flex flex-col min-h-screen">
    ...
    <nav class="bg-gray-900 border-b border-gray-800 px-6 py-4 flex justify-between items-center shrink-0">
        ...
        <a href="{{ route('admin.subir') }}" class="text-xs text-gray-400 hover:text-white uppercase tracking-wider font-bold transition-colors">
            ← Volver al Listado
        </a>
    </nav>
    ...
    <main class="flex-grow flex items-center justify-center py-12 px-6">
        <div class="max-w-2xl w-full bg-gray-900 p-8 rounded-3xl border border-gray-800 space-y-6">
            ...
            <form action="{{ route('portafolio.update', $trabajo->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                
                <div>
                    <label class="block text-xs font-bold uppercase text-gray-400 mb-2">Título del Proyecto</label>
                    <input type="text" name="titulo" value="{{ old('titulo', $trabajo->titulo) }}" class="w-full bg-black border border-gray-800 rounded-xl p-3 text-white focus:border-orange-500 outline-none text-sm" required>
                </div>
                ...
                <div class="p-4 bg-black/40 border border-gray-800/60 rounded-2xl grid grid-cols-1 sm:grid-cols-4 gap-4 items-center">
                    <div class="sm:col-span-1">
                        ...
                        <img src="{{ asset('storage/' . $trabajo->caratula) }}" class="w-16 h-16 object-cover rounded-xl border border-gray-800">
                    </div>
                ...
                <div class="flex gap-4 pt-2">
                    <a href="{{ route('admin.subir') }}" class="w-1/3 bg-gray-800 hover:bg-gray-700 text-gray-300 font-bold text-xs py-4 rounded-xl uppercase tracking-wider text-center transition-colors">
                        Cancelar
                    </a>
